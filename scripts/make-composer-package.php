...
$composer_create_zip = false;
if (array_key_exists('composer_create_zip', $ini)) {
    $composer_create_zip = (bool)$ini['composer_create_zip'];
}
$vendor_name = $ini['vendor_name'];

// MAKE A ZIP FILE
if ($composer_create_zip) {
    // check for zip
    ...
    $package_file_name = "composer-{$package_name}-{$release}.zip";
    $command = 'zip -r ../' . $package_file_name;
...
$composer['dist']['url'] = "http://packages.zendframework.com/get/$package_file_name";
